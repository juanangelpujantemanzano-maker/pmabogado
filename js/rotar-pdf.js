document.addEventListener("DOMContentLoaded", function () {
  ...
  var rotateAllLeftBtn = document.getElementById("rotateAllLeftBtn");
  var rotateAllRightBtn = document.getElementById("rotateAllRightBtn");
  ...
  var PAGE_TARGET_WIDTH = 260; // ancho de render de cada miniatura, en px

  var currentFile = null;
  var originalBytes = null;
  var documentPassword = null;
  var totalPages = 0;
  var pageInfos = []; // { extraRotation }
  ...
  // Pide la contraseña del documento mediante el mismo cuadro que usan el
  // resto de herramientas; puede invocarse varias veces si la contraseña
  // introducida no es correcta (callback onPassword de pdf.js).
  ...
  function renderThumbnails(pdf) {
      ...
      chain = chain.then(function () {
        return pdf.getPage(pageNum).then(function (page) {
          ...
          return page.render({ canvasContext: ctx, viewport: viewport }).promise.then(function () {
            ...
            leftBtn.type = "button";
            leftBtn.innerHTML = "&#8634;";
            leftBtn.setAttribute("aria-label", "Rotar página " + pageNum + " a la izquierda");
            leftBtn.addEventListener("click", function () { rotatePage(idx, -90, canvas); });
            actions.appendChild(leftBtn);

            var rightBtn = document.createElement("button");
            rightBtn.type = "button";
            rightBtn.innerHTML = "&#8635;";
            rightBtn.setAttribute("aria-label", "Rotar página " + pageNum + " a la derecha");
            rightBtn.addEventListener("click", function () { rotatePage(idx, 90, canvas); });
    ...
    return chain.then(function () {
      rotateArea.style.display = "block";
      hidePasswordBox();
      setStatus("");
    });
  }

  function rotatePage(idx, delta, canvas) {
    var info = pageInfos[idx];
    info.extraRotation = ((info.extraRotation + delta) % 360 + 360) % 360;
    canvas.style.transform = "rotate(" + info.extraRotation + "deg)";
  }

  rotateAllLeftBtn.addEventListener("click", function () {
    var canvases = pageGrid.querySelectorAll(".page-thumb-canvas-wrap canvas");
    canvases.forEach(function (canvas, idx) { rotatePage(idx, -90, canvas); });
  });
  ...
  // ---------- Generación del PDF rotado ----------

  async function generateRotated() {
    generateBtn.disabled = true;
    resultBox.style.display = "none";
    setStatus("Rotando documento…");
    ...
    try {
      ...
      pages.forEach(function (page, idx) {
        var extra = pageInfos[idx] ? pageInfos[idx].extraRotation : 0;
        if (!extra) return;
        var current = page.getRotation().angle;
        var newAngle = ((current + extra) % 360 + 360) % 360;
      ...
      var outName = currentFile.name.replace(/\.pdf$/i, "") + "_rotado.pdf";
      ...
      console.error(err);
      setStatus("No se pudo rotar el archivo: " + (err && err.message ? err.message : "error desconocido") + ".", true);
    } finally {
      generateBtn.disabled = false;
    }
  }

  generateBtn.addEventListener("click", generateRotated);
});
